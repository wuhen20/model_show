...
// 隐藏的字段（不需要在表格中展示）
const hiddenColumns = new Set(['recordId', 'sampleNo', 'typeCode', 'typeName', 'dirId', 'fileName', 'labelFlagCode', 'labelFlag', 'sampleScore', 'labelThink', 'labelContent', 'updateTime'])

// el-table 动态列：过滤掉隐藏列，并保证 filePath 排在最后
const visibleColumns = computed(() => {
  const visible = columns.value.filter(c => !hiddenColumns.has(c.key))
  visible.sort((a, b) => {
      ...
      <main class="content-area">
        ...
        <div class="table-wrapper" v-if="isTimeSeriesSet || (!isImageSet || viewMode === 'list')" v-loading="loading">
          <el-table :data="pagedList" style="width: 100%" class="sample-table">
            <el-table-column type="index" label="#" width="50" :index="(i: number) => (isTimeSeriesSet ? tsCurrentPage - 1 : currentPage - 1) * (isTimeSeriesSet ? tsPageSize : pageSize) + i + 1" />
