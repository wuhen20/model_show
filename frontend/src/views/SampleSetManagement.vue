...
import { getCodeDict, saveSampleSet, updateSampleSet, querySampleSet, uploadSamples, uploadSamplesBatch, getDirectoryTree, deleteSampleSet, type CodeDictItem, type DirectoryNode } from '@/api/sample'
...
async function handleDelete(item: SampleSet) {
  try {
    await ElMessageBox.confirm('确定要删除该样本集吗？仅允许删除空样本集（无样本）。', '提示', {
  ...
  try {
    await deleteSampleSet(item.setNo)
        ...
        <div v-else class="list-view">
          <el-table :data="filteredData" style="width: 100%" class="sample-set-table">
            ...
            <el-table-column prop="qualityLevel" label="质量等级" width="100">
              <template #default="{ row }">
                <span class="quality-badge-sm" :style="{ color: qualityColor[row.qualityLevel], borderColor: qualityColor[row.qualityLevel] }">{{ qualityLabel[row.qualityLevel] }}</span>
              </template>
            </el-table-column>
